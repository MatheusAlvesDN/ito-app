...
export default function RegisterScreenWhatDoYouKnow({ onBack, onNext }: Props) {
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-955 relative h-full overflow-hidden text-slate-100 font-sans">
      ...
      <div className="p-4 flex items-center bg-slate-900/60 border-b border-white/5 backdrop-blur-md sticky top-0 z-20 pt-8 md:pt-4 safe-top shrink-0">
        ...
        <span className="ml-4 font-black text-xl text-white font-outfit">Jogadores (O Que Você Sabe?)</span>
      </div>
      ...
      <div className="flex-1 p-6 flex flex-col max-w-full overflow-hidden">
        ...
        <div className="bg-slate-900/40 p-5 rounded-3xl border border-white/5 shadow-xl mb-6 shrink-0 animate-fade-in-scale">
          <label className="block text-rose-400 font-bold mb-2 ml-1 text-xs uppercase tracking-wider font-outfit">Novo Participante</label>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-slate-955 border border-white/5 rounded-2xl px-4 py-3.5 text-white placeholder:text-slate-500 focus:ring-2 focus:ring-rose-500 focus:border-rose-500 outline-none transition-all duration-200"
            />
            <button 
              onClick={addPlayer} 
              disabled={!inputValue.trim()} 
              className="bg-rose-600 hover:bg-rose-500 disabled:bg-slate-900 disabled:text-slate-600 text-white rounded-2xl p-3.5 shadow-md active:scale-95 disabled:scale-100 transition-all duration-150"
            >
              <Plus size={24} />
            </button>
          </div>
        ...
        {savedHistory.length > 0 && (
          <div className="mb-6 shrink-0 animate-fade-in">
            <span className="block text-rose-400 font-bold mb-2 ml-1 text-[11px] uppercase tracking-wider font-outfit">Rápida Seleção / Histórico</span>
                  ...
                  <div
                    ...
                    className={`flex items-center gap-1.5 px-3.5 py-2 rounded-2xl border text-xs font-bold transition-all duration-200 cursor-pointer select-none active:scale-95 ${
                      isSelected
                        ...
                        : 'bg-slate-900/40 border border-white/5 text-slate-300 hover:border-rose-500/30'
                    }`}
                  >
                    <span>{player}</span>
                    <button
                      ...
                      className={`p-0.5 rounded-full transition-colors ${
                        isSelected ? 'hover:bg-rose-750 text-rose-200' : 'hover:bg-slate-800 text-slate-500'
                      }`}
          ...
          {localPlayers.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-48 text-slate-500 animate-fade-in">
              <div className="w-16 h-16 bg-slate-900/60 rounded-full flex items-center justify-center mb-3 border border-white/5">
                <HelpCircle size={32} className="text-rose-400 opacity-60 animate-pulse" />
              </div>
              <p className="font-bold text-slate-400 text-sm">Adicione pelo menos 3 jogadores</p>
              <p className="text-xs text-slate-550 mt-1">Quem conhece melhor quem?</p>
            </div>
          ) : (
            localPlayers.map((player, index) => (
              <div key={`${player}-${index}`} className="flex items-center justify-between bg-slate-900/30 p-4 rounded-2xl border border-white/5 hover:border-rose-500/30 transition-all duration-200 animate-fade-in">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-gradient-to-br from-rose-400 to-pink-600 rounded-full flex items-center justify-center text-white font-black text-sm shadow-sm select-none">
