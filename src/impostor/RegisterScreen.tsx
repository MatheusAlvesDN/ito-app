  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-955 relative h-full overflow-hidden text-slate-100 font-sans">
      ...
      <div className="p-4 flex items-center bg-slate-900/60 border-b border-white/5 backdrop-blur-md sticky top-0 z-20 pt-8 md:pt-4 safe-top shrink-0">
        <button aria-label="Voltar" onClick={onBack} className="p-2 bg-white/5 rounded-full hover:bg-white/10 text-purple-400 transition-colors">
          <ChevronLeft size={24} />
        </button>
      ...
      <div className="flex-1 p-6 flex flex-col max-w-full overflow-hidden">
        ...
        <div className="bg-slate-900/40 p-5 rounded-3xl border border-white/5 shadow-xl mb-6 shrink-0 animate-fade-in-scale">
          <label htmlFor="new-player" className="block text-purple-400 font-bold mb-2 ml-1 text-xs uppercase tracking-wider font-outfit">Novo Participante</label>
          ...
          {localPlayers.length === 0 ? (
            ...
            localPlayers.map((player, index) => (
              <div key={`${player}-${index}`} className="flex items-center justify-between bg-slate-900/30 p-4 rounded-2xl border border-white/5 hover:border-purple-500/30 transition-all duration-200 animate-fade-in">
                ...
                <button aria-label="Remover jogador" onClick={() => removePlayer(index)} className="p-2 text-slate-500 hover:text-red-400 hover:bg-red-500/10 rounded-xl transition-all duration-150">
                  <Trash2 size={20} />
                </button>
              </div>
            ))
