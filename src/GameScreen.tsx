                        ...
                        return (
                            <div key={player} className={`flex items-center p-3 rounded-xl border-2 transition-all duration-500 ${phase === 'result' ? (isWrong ? 'bg-red-900/40 border-red-500/50' : 'bg-slate-800 border-green-500/30') : 'bg-slate-800 border-slate-700 shadow-md'}`}>
                                <div className="flex flex-col gap-1 mr-3">
                                    {phase === 'ordering' && (
                                      <>
                                        <button onClick={() => movePlayer(index, 'up')} disabled={index === 0} aria-label="Mover para cima" className="p-1 text-slate-500 hover:text-white disabled:opacity-0 focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-slate-400 rounded"><ArrowUp size={20} /></button>
                                        <button onClick={() => movePlayer(index, 'down')} disabled={index === orderedPlayers.length - 1} aria-label="Mover para baixo" className="p-1 text-slate-500 hover:text-white disabled:opacity-0 focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-slate-400 rounded"><ArrowDown size={20} /></button>
                                      </>
                                    )}
      ...
      <div className="p-6 bg-slate-900 border-t border-slate-800 z-40 absolute bottom-0 w-full shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
        ...
        {phase === 'result' && (
            <div className="flex gap-3">
                 ...
                 <button onClick={nextRound} aria-label="Nova rodada" className="bg-slate-700 hover:bg-slate-600 text-white p-4 rounded-2xl shadow-lg active:scale-95 focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-slate-400">
                    <RefreshCw size={28} />
                </button>
            </div>
        )}
