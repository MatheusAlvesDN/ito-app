...
const GameScreen = ({ onBack, players, themes }: { onBack: () => void, players: string[], themes: Theme[] }) => {
  ...
  const startOrdering = () => {
    ...
    if (isFreeMode) {
      ...
      // ⚡ Bolt: Optimize random item selection without flatMap to save memory.
      // We calculate total questions and iterate to pick one with O(C) memory overhead.
      ...
      if (totalQuestions > 0) {
        ...
        for (const t of themes) {
          const qs = QUESTIONS_DB[t.id] || [];
          if (randomIndex < qs.length) {
            selectedQuestion = qs[randomIndex];
            selectedTheme = t;
            break;
          }
