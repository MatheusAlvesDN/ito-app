                        ...
                        return (
                            <div key={player} className={`flex items-center p-3 rounded-xl border-2 transition-all duration-500 ${phase === 'result' ? (isWrong ? 'bg-red-900/40 border-red-500/50' : 'bg-slate-800 border-green-500/30') : 'bg-slate-800 border-slate-700 shadow-md'}`}>
                                <div className="flex flex-col gap-1 mr-3">
                                    {phase === 'ordering' && (
                                      <>
                                        <button onClick={() => movePlayer(index, 'up')} disabled={index === 0} aria-label="Mover jogador para cima" className="p-1 text-slate-500 hover:text-white disabled:opacity-0 focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-white rounded"><ArrowUp size={20} /></button>
                                        <button onClick={() => movePlayer(index, 'down')} disabled={index === orderedPlayers.length - 1} aria-label="Mover jogador para baixo" className="p-1 text-slate-500 hover:text-white disabled:opacity-0 focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-white rounded"><ArrowDown size={20} /></button>
                                      </>
                                    )}
