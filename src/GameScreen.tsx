  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-900 text-white relative h-full">
      ...
      <div className={`p-4 flex items-center justify-between backdrop-blur-md sticky top-0 z-20 bg-slate-900/80 border-b border-white/5 pt-8 md:pt-4`}>
        <button onClick={onBack} aria-label="Voltar" className="p-2 bg-white/10 hover:bg-white/20 rounded-full text-white focus-visible:ring-2 focus-visible:ring-white focus-visible:outline-none"><ChevronLeft size={24} /></button>
                        ...
                        return (
                            <div key={player} className={`flex items-center p-3 rounded-xl border-2 transition-all duration-500 ${phase === 'result' ? (isWrong ? 'bg-red-900/40 border-red-500/50' : 'bg-slate-800 border-green-500/30') : 'bg-slate-800 border-slate-700 shadow-md'}`}>
                                <div className="flex flex-col gap-1 mr-3">
                                    {phase === 'ordering' && (
                                      <>
                                        <button onClick={() => movePlayer(index, 'up')} disabled={index === 0} aria-label={`Mover ${player} para cima`} className="p-1 text-slate-500 hover:text-white disabled:opacity-0 focus-visible:ring-2 focus-visible:ring-white focus-visible:outline-none rounded"><ArrowUp size={20} /></button>
                                        <button onClick={() => movePlayer(index, 'down')} disabled={index === orderedPlayers.length - 1} aria-label={`Mover ${player} para baixo`} className="p-1 text-slate-500 hover:text-white disabled:opacity-0 focus-visible:ring-2 focus-visible:ring-white focus-visible:outline-none rounded"><ArrowDown size={20} /></button>
                                      </>
                                    )}
