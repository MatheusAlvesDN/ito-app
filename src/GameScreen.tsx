  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-900 text-white relative h-full">
      ...
      <div className={`p-4 flex items-center justify-between backdrop-blur-md sticky top-0 z-20 bg-slate-900/80 border-b border-white/5 pt-8 md:pt-4`}>
        <button onClick={onBack} aria-label="Voltar para seleção de temas" className="p-2 bg-white/10 hover:bg-white/20 rounded-full text-white"><ChevronLeft size={24} /></button>
      ...
      <div className="p-6 bg-slate-900 border-t border-slate-800 z-40 absolute bottom-0 w-full shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
        ...
        {phase === 'result' && (
            <div className="flex gap-3">
                 ...
                 <button onClick={nextRound} aria-label="Iniciar próxima rodada" className="bg-slate-700 hover:bg-slate-600 text-white p-4 rounded-2xl shadow-lg active:scale-95">
                    <RefreshCw size={28} />
                </button>
            </div>
        )}
