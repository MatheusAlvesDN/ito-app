...
const Confetti = () => {
  ...
  // 🛡️ Sentinel: Use a lazy initializer function in useState to satisfy React purity rules
  // while allowing Math.random() for purely visual effects that don't affect gameplay logic.
  ...
  useEffect(() => {
    if (phase === 'rolling') {
      const timeout = setTimeout(() => {
        ...
        players.forEach(p => {
            let num;
            // 🛡️ Sentinel: Use getSecureRandomInt instead of Math.random() for logical random assignments to avoid predictability.
