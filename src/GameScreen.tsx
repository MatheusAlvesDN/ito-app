...
const GameScreen = ({ onBack, players, themes }: { onBack: () => void, players: string[], themes: Theme[] }) => {
  ...
  const startOrdering = () => {
    ...
    if (isFreeMode) {
      ...
      // ⚡ Bolt: Optimize random question selection to save memory
      // Avoid flatMap which creates a new array allocating O(N) space.
      // Instead count total questions, pick global random index, and iterate to find it.
