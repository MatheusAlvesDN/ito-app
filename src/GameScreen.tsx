...
const GameScreen = ({ onBack, players, themes }: { onBack: () => void, players: string[], themes: Theme[] }) => {
  ...
  const startOrdering = () => {
    ...
    if (isFreeMode) {
      ...
      // ⚡ Bolt Optimization: Calculate total questions and select by index to avoid flatMap and includes() overhead
      ...
      if (totalQuestions > 0) {
        ...
        for (let i = 0; i < themes.length; i++) {
          ...
          if (randomIndex < qList.length) {
            setCurrentQuestion(qList[randomIndex]);
            setCurrentThemeColor(t.color);
            break;
          }
          randomIndex -= qList.length;
        }
      }
    }
    setPhase('ordering');
