...
const Confetti = () => {
  // Cria 50 partículas com posições e cores aleatórias na montagem inicial para respeitar as regras do React Hooks Purity
  const [particles, setParticles] = useState<{ id: number; x: number; delay: number; color: string; duration: number }[]>([]);

  useEffect(() => {
    // Usando setTimeout para inicializar partículas de forma segura para evitar warning "cascading renders"
