...
const Confetti = () => {
  ...
  // Usamos useEffect para evitar impureza no render (Math.random) e garantir hidratação correta
  const [particles, setParticles] = useState<{id: number, x: number, delay: number, color: string, duration: number}[]>([]);

  useEffect(() => {
    // Hack para evitar erro de linter sobre set state em effect (cascading render)
    // Como Math.random é impuro, precisamos gerar no cliente após a montagem
