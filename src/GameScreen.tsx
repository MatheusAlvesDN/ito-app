...
const Confetti = () => {
  // Cria 50 partículas com posições e cores aleatórias usando state lazy initialization para evitar rerenders e logs de impureza
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-900 text-white relative h-full">
      ...
      <div className={`p-4 flex items-center justify-between backdrop-blur-md sticky top-0 z-20 bg-slate-900/80 border-b border-white/5 pt-8 md:pt-4`}>
        <button onClick={onBack} aria-label="Voltar" className="p-2 bg-white/10 hover:bg-white/20 rounded-full text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow-400"><ChevronLeft size={24} /></button>
                        ...
                        return (
                            <div key={player} className={`flex items-center p-3 rounded-xl border-2 transition-all duration-500 ${phase === 'result' ? (isWrong ? 'bg-red-900/40 border-red-500/50' : 'bg-slate-800 border-green-500/30') : 'bg-slate-800 border-slate-700 shadow-md'}`}>
                                <div className="flex flex-col gap-1 mr-3">
                                    {phase === 'ordering' && (
                                      <>
                                        <button onClick={() => movePlayer(index, 'up')} aria-label={`Mover ${player} para cima`} disabled={index === 0} className="p-1 text-slate-500 hover:text-white disabled:opacity-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow-400 rounded"><ArrowUp size={20} /></button>
                                        <button onClick={() => movePlayer(index, 'down')} aria-label={`Mover ${player} para baixo`} disabled={index === orderedPlayers.length - 1} className="p-1 text-slate-500 hover:text-white disabled:opacity-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow-400 rounded"><ArrowDown size={20} /></button>
                                      </>
                                    )}
      ...
      <div className="p-6 bg-slate-900 border-t border-slate-800 z-40 absolute bottom-0 w-full shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
        ...
        {phase === 'result' && (
            <div className="flex gap-3">
                 ...
                 <button onClick={nextRound} aria-label="Próxima rodada" className="bg-slate-700 hover:bg-slate-600 text-white p-4 rounded-2xl shadow-lg active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow-400">
                    <RefreshCw size={28} />
                </button>
            </div>
        )}
