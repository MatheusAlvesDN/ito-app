...
const GameScreen = ({ onBack, players, themes }: { onBack: () => void, players: string[], themes: Theme[] }) => {
  ...
  const startOrdering = () => {
    ...
    if (isFreeMode) {
      ...
      // ⚡ Bolt Optimization: Removed memory allocation from flatMap and O(N) reverse-lookup logic
      // Calculating the total count and navigating the array of themes sequentially guarantees
      // O(T) speed (where T is the number of themes) and O(1) space complexity.
