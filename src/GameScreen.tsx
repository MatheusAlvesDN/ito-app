  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-900 text-white relative h-full">
      ...
      <div className={`p-4 flex items-center justify-between backdrop-blur-md sticky top-0 z-20 bg-slate-900/80 border-b border-white/5 pt-8 md:pt-4`}>
        <button aria-label="Voltar" onClick={onBack} className="p-2 bg-white/10 hover:bg-white/20 rounded-full text-white focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-white"><ChevronLeft size={24} /></button>
          ...
          <div className="animate-fade-in pb-24">
            ...
            <div className="grid grid-cols-2 gap-4">
                {players.map((player) => {
                    ...
                    return (
                      <button aria-label={`Revelar número de ${player}`} key={player} onClick={() => !hasSeen && setViewingPlayer(player)} disabled={hasSeen} className={`p-4 rounded-2xl border transition-all flex flex-col items-center justify-center h-32 relative overflow-hidden focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-yellow-400 ${hasSeen ? 'bg-slate-800/40 border-slate-800 opacity-50' : 'bg-slate-800 border-slate-600 hover:border-yellow-400 shadow-lg active:scale-95'}`}>
                        ...
                        return (
                            <div key={player} className={`flex items-center p-3 rounded-xl border-2 transition-all duration-500 ${phase === 'result' ? (isWrong ? 'bg-red-900/40 border-red-500/50' : 'bg-slate-800 border-green-500/30') : 'bg-slate-800 border-slate-700 shadow-md'}`}>
                                <div className="flex flex-col gap-1 mr-3">
                                    {phase === 'ordering' && (
                                      <>
                                        <button aria-label={`Mover ${orderedPlayers[index]} para cima`} onClick={() => movePlayer(index, 'up')} disabled={index === 0} className="p-1 text-slate-500 hover:text-white disabled:opacity-0 focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-yellow-400"><ArrowUp size={20} /></button>
                                        <button aria-label={`Mover ${orderedPlayers[index]} para baixo`} onClick={() => movePlayer(index, 'down')} disabled={index === orderedPlayers.length - 1} className="p-1 text-slate-500 hover:text-white disabled:opacity-0 focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-yellow-400"><ArrowDown size={20} /></button>
                                      </>
                                    )}
      ...
      <div className="p-6 bg-slate-900 border-t border-slate-800 z-40 absolute bottom-0 w-full shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
        ...
        {phase === 'result' && (
            <div className="flex gap-3">
                 ...
                 <button aria-label="Próxima rodada" onClick={nextRound} className="bg-slate-700 hover:bg-slate-600 text-white p-4 rounded-2xl shadow-lg active:scale-95 focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-white">
                    <RefreshCw size={28} />
                </button>
            </div>
        )}
