...
const GameScreen = ({ onBack, players, themes }: { onBack: () => void, players: string[], themes: Theme[] }) => {
  ...
  const startOrdering = () => {
    ...
    if (isFreeMode) {
      ...
      // Opt: calculate total count instead of allocating a large merged array via flatMap
      ...
      if (totalQuestions > 0) {
          // Choose a random global index
          let globalIndex = Math.floor(Math.random() * totalQuestions);

          // Locate the question without a reverse lookup
          for (let i = 0; i < themes.length; i++) {
            const tQuestions = QUESTIONS_DB[themes[i].id] || [];
            if (globalIndex < tQuestions.length) {
              setCurrentQuestion(tQuestions[globalIndex]);
              setCurrentThemeColor(themes[i].color);
              break;
            }
            globalIndex -= tQuestions.length;
          }
      }
    }
    setPhase('ordering');
