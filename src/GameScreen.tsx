  ...
  const startOrdering = () => {
    ...
    if (isFreeMode) {
      ...
      if (totalQuestions > 0) {
        ...
        let selectedThemeColor = themes[0].color;

        for (const t of themes) {
          const themeQuestions = QUESTIONS_DB[t.id] || [];
          if (randomIndex < themeQuestions.length) {
            selectedQuestion = themeQuestions[randomIndex];
            selectedThemeColor = t.color;
            break;
          }
          randomIndex -= themeQuestions.length;
        }

        setCurrentQuestion(selectedQuestion);
        setCurrentThemeColor(selectedThemeColor);
      }
    }
    setPhase('ordering');
  };
