  ...
  useEffect(() => {
    const lockOrientation = async () => {
      try {
        ...
        // @ts-expect-error Typescript definitions for window.screen.orientation are not fully implemented
...
// Componente isolado para o input de adicionar jogador, previne re-renderização da lista
const AddPlayerInput = ({ onAdd }: { onAdd: (name: string) => void }) => {
  ...
  return (
    <div className="bg-white p-4 rounded-3xl shadow-lg mb-6 border border-slate-100 shrink-0">
      <label className="block text-slate-500 font-bold mb-2 ml-1 text-xs uppercase tracking-wider" htmlFor="add-player-input">
        Adicionar Jogador
      </label>
