...
const RegisterScreen = ({ onBack, onNext }: { onBack: () => void, onNext: (players: string[]) => void }) => {
  ...
  const addPlayer = () => {
    // SECURITY: Limit maximum players to 20 to prevent infinite loop DoS in GameScreen's random number generation
        ...
        <div className="bg-white p-4 rounded-3xl shadow-lg mb-6 border border-slate-100 shrink-0">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder={localPlayers.length >= 20 ? "Limite máximo de jogadores atingido" : "Nome do participante"}
              maxLength={30}
