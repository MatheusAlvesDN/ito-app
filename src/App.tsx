...
const RegisterScreen = ({ onBack, onNext }: { onBack: () => void, onNext: (players: string[]) => void }) => {
  ...
  const addPlayer = () => {
    // 🛡️ SECURITY: Prevent infinite loop DoS in GameScreen's random number assignment
    // by restricting the number of players to 20 (must be significantly smaller than pool of 100).
        ...
        <div className="bg-white p-4 rounded-3xl shadow-lg mb-6 border border-slate-100 shrink-0">
          ...
          <div className="flex gap-2">
            <input
              ...
              maxLength={30} // 🛡️ SECURITY: Prevent memory exhaustion from extremely long strings
