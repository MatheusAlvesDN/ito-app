...
const RegisterScreen = ({ onBack, onNext }: { onBack: () => void, onNext: (players: string[]) => void }) => {
  ...
  const addPlayer = () => {
    // 🛡️ SECURITY: Prevent memory exhaustion and infinite loop DoS (do-while max 100) by capping players at 20
        ...
        <div className="bg-white p-4 rounded-3xl shadow-lg mb-6 border border-slate-100 shrink-0">
          ...
          <div className="flex gap-2">
            <input
              ...
              // 🛡️ SECURITY: Prevent long input string memory exhaustion and UI overflow
              maxLength={30}
