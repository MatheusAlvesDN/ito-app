...
const RegisterScreen = ({ onBack, onNext }: { onBack: () => void, onNext: (players: string[]) => void }) => {
  ...
  const addPlayer = () => {
    // 🛡️ SECURITY: Prevent memory exhaustion and infinite loop DoS
    // GameScreen loops up to 100 times to assign unique numbers.
    // We strictly limit to 20 players to avoid the loop spinning infinitely.
        ...
        <div className="bg-white p-4 rounded-3xl shadow-lg mb-6 border border-slate-100 shrink-0">
          ...
          <div className="flex gap-2">
            <input
              ...
              maxLength={30} // 🛡️ SECURITY: Prevent excessively large input strings
