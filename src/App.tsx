  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 relative h-full">
      <div className="p-4 flex items-center bg-white shadow-sm sticky top-0 z-20 pt-8 md:pt-4 safe-top">
        <button onClick={onBack} aria-label="Voltar" className="p-2 bg-slate-100 rounded-full hover:bg-slate-200 focus-visible:ring-2 focus-visible:outline-none transition-colors"><ChevronLeft size={24} className="text-slate-700" /></button>
      ...
      <div className="flex-1 p-6 flex flex-col max-w-full overflow-hidden">
        <div className="bg-white p-4 rounded-3xl shadow-lg mb-6 border border-slate-100 shrink-0">
          <label htmlFor="player-input" className="block text-slate-500 font-bold mb-2 ml-1 text-xs uppercase tracking-wider">Adicionar Jogador</label>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-slate-100 border-none rounded-xl px-4 py-3 text-slate-900 focus:ring-2 focus:ring-yellow-400 focus-visible:ring-2 focus-visible:outline-none transition-all"
            />
            <button onClick={addPlayer} disabled={!inputValue.trim()} aria-label="Adicionar Jogador" className="bg-sky-500 hover:bg-sky-400 disabled:bg-slate-200 disabled:text-slate-400 text-white rounded-xl p-3 shadow-md focus-visible:ring-2 focus-visible:outline-none transition-all active:scale-95">
              <Plus size={24} />
            </button>
          </div>
        </div>
          ...
          {localPlayers.length === 0 ? (
            ...
            localPlayers.map((player, index) => (
              <div key={index} className="flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-slate-100 animate-fade-in">
                ...
                <button onClick={() => { const n = [...localPlayers]; n.splice(index, 1); setLocalPlayers(n); }} aria-label={`Remover ${player}`} className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-full focus-visible:ring-2 focus-visible:outline-none transition-colors">
                  <Trash2 size={20} />
                </button>
              </div>
            ))
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 relative h-full">
        ...
        {THEMES.map((theme) => {
          ...
          return (
            <button
              ...
              className={`w-full group text-left relative overflow-hidden rounded-3xl p-6 shadow-md focus-visible:ring-2 focus-visible:outline-none transition-all duration-200 border-2 ${isSelected ? 'bg-white border-sky-500 ring-4 ring-sky-100 scale-[1.02]' : 'bg-white border-transparent hover:border-slate-200'}`}
            >
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 bg-white border-t border-slate-100 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] safe-bottom">
        <button onClick={() => onStart(THEMES.filter(t => selectedIds.includes(t.id)))} disabled={selectedIds.length === 0} aria-label="Iniciar Jogo" className="w-full bg-yellow-400 hover:bg-yellow-300 disabled:bg-slate-200 disabled:text-slate-400 text-black font-black text-xl py-4 rounded-2xl shadow-lg focus-visible:ring-2 focus-visible:outline-none transition-all active:scale-95 flex items-center justify-center gap-3">
