  ...
  useEffect(() => {
    const lockOrientation = async () => {
      try {
        ...
        // @ts-expect-error - window.screen.orientation types are incomplete in this environment
        if (window.screen && window.screen.orientation && typeof window.screen.orientation.lock === 'function') {
          // @ts-expect-error - window.screen.orientation.lock types are incomplete in this environment
...
const RegisterScreen = ({ onBack, onNext }: { onBack: () => void, onNext: (players: string[]) => void }) => {
  ...
  const addPlayer = () => {
    ...
    if (name && localPlayers.length < 20 && !localPlayers.includes(name) && name.length <= 20) {
