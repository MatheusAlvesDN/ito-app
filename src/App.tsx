  ...
  useEffect(() => {
    const lockOrientation = async () => {
      try {
        ...
        // @ts-expect-error - TS definition for window.screen.orientation.lock is incomplete
...
const RegisterScreen = ({ onBack, onNext }: { onBack: () => void, onNext: (players: string[]) => void }) => {
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 relative h-full">
      ...
      <div className="flex-1 p-6 flex flex-col max-w-full overflow-hidden" aria-live="polite">
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 relative h-full">
      <div className="p-4 flex items-center bg-white/90 backdrop-blur-md sticky top-0 z-20 shadow-sm pt-8 md:pt-4 safe-top">
        <button onClick={onBack} aria-label="Voltar para a tela anterior" className="p-2 bg-slate-100 rounded-full hover:bg-slate-200 transition-colors"><ChevronLeft size={24} className="text-slate-700" /></button>
        <div className="ml-4">
