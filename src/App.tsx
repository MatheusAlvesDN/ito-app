  ...
  useEffect(() => {
    const lockOrientation = async () => {
      try {
        ...
        // @ts-expect-error - The screen orientation API is non-standard or missing in typescript libs
...
const RegisterScreen = ({ onBack, onNext }: { onBack: () => void, onNext: (players: string[]) => void }) => {
  ...
  // 🛡️ Sentinel: Enforce player limit and name length to prevent memory exhaustion and GameScreen DoS (infinite loop if > 100 players)
  const MAX_PLAYERS = 20;
  const MAX_NAME_LENGTH = 20;

  const addPlayer = () => {
    const name = inputValue.trim();
    if (name && localPlayers.length < MAX_PLAYERS && name.length <= MAX_NAME_LENGTH) {
      setLocalPlayers([...localPlayers, name]);
      setInputValue('');
    }
  };
    ...
    <div className="flex-1 flex flex-col bg-slate-50 relative h-full">
      ...
      <div className="flex-1 p-6 flex flex-col max-w-full overflow-hidden">
        <div className="bg-white p-4 rounded-3xl shadow-lg mb-6 border border-slate-100 shrink-0">
          <label className="block text-slate-500 font-bold mb-2 ml-1 text-xs uppercase tracking-wider">Adicionar Jogador {localPlayers.length >= MAX_PLAYERS && <span className="text-red-500 ml-2">(Máximo de {MAX_PLAYERS} atingido)</span>}</label>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-slate-100 border-none rounded-xl px-4 py-3 text-slate-900 focus:ring-2 focus:ring-yellow-400 outline-none transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            />
            <button onClick={addPlayer} disabled={!inputValue.trim() || localPlayers.length >= MAX_PLAYERS} className="bg-sky-500 hover:bg-sky-400 disabled:bg-slate-200 disabled:text-slate-400 text-white rounded-xl p-3 shadow-md transition-all active:scale-95">
              <Plus size={24} />
            </button>
          </div>
        </div>
