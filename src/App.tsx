...
const RegisterScreen = ({ onBack, onNext }: { onBack: () => void, onNext: (players: string[]) => void }) => {
  ...
  const addPlayer = () => {
    // 🛡️ Sentinel: Prevent memory exhaustion / Infinite loop DoS if more than 100 players
    if (inputValue.trim() && localPlayers.length < 20) {
      // 🛡️ Sentinel: Limit max player length
      setLocalPlayers([...localPlayers, inputValue.trim().slice(0, 30)]);
      setInputValue('');
    }
  };
      ...
      <div className="flex-1 p-6 flex flex-col max-w-full overflow-hidden">
        <div className="bg-white p-4 rounded-3xl shadow-lg mb-6 border border-slate-100 shrink-0">
          <label className="block text-slate-500 font-bold mb-2 ml-1 text-xs uppercase tracking-wider">
            Adicionar Jogador {localPlayers.length >= 20 && <span className="text-red-500 ml-1">(Máx. 20)</span>}
          </label>
