  ...
  useEffect(() => {
    const lockOrientation = async () => {
      try {
        ...
        // @ts-expect-error - Screen Orientation API might not be fully typed or available in all environments
        if (window.screen && window.screen.orientation && typeof window.screen.orientation.lock === 'function') {
          // @ts-expect-error - Screen Orientation API might not be fully typed
    ...
    <div className="flex-1 flex flex-col bg-slate-50 relative h-full">
      ...
      <div className="flex-1 p-6 flex flex-col max-w-full overflow-hidden">
        <div className="bg-white p-4 rounded-3xl shadow-lg mb-6 border border-slate-100 shrink-0">
          <label htmlFor="add-player" className="block text-slate-500 font-bold mb-2 ml-1 text-xs uppercase tracking-wider">Adicionar Jogador</label>
