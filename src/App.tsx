  ...
  useEffect(() => {
    const lockOrientation = async () => {
      try {
        ...
        // @ts-expect-error - Screen API types may not be fully complete in older TS versions
        if (window.screen && window.screen.orientation && typeof window.screen.orientation.lock === 'function') {
          // @ts-expect-error - lock() may not exist on all ScreenOrientation interfaces
...
const RegisterScreen = ({ onBack, onNext }: { onBack: () => void, onNext: (players: string[]) => void }) => {
  ...
  const addPlayer = () => {
    ...
    if (trimmed && trimmed.length <= 50 && localPlayers.length < 20) {
      setLocalPlayers([...localPlayers, trimmed]);
      setInputValue('');
    }
  };

  const isMaxPlayers = localPlayers.length >= 20;
      ...
      <div className="flex-1 p-6 flex flex-col max-w-full overflow-hidden">
        <div className="bg-white p-4 rounded-3xl shadow-lg mb-6 border border-slate-100 shrink-0">
          <label className="block text-slate-500 font-bold mb-2 ml-1 text-xs uppercase tracking-wider">
            Adicionar Jogador {isMaxPlayers && <span className="text-red-500 ml-1">(Máx 20)</span>}
          </label>
          <div className="flex gap-2">
            <input
              ...
              placeholder={isMaxPlayers ? "Limite máximo atingido" : "Nome do participante"}
              maxLength={50}
              disabled={isMaxPlayers}
              className="flex-1 bg-slate-100 border-none rounded-xl px-4 py-3 text-slate-900 focus:ring-2 focus:ring-yellow-400 outline-none transition-all disabled:opacity-50 disabled:bg-slate-200"
            />
            <button
              onClick={addPlayer}
              disabled={!inputValue.trim() || isMaxPlayers}
