...
const RegisterScreen = ({ onBack, onNext }: { onBack: () => void, onNext: (players: string[]) => void }) => {
  ...
  const addPlayer = () => {
    // 🛡️ SECURITY: Prevent memory exhaustion DoS in GameScreen do...while loop
        ...
        <div className="bg-white p-4 rounded-3xl shadow-lg mb-6 border border-slate-100 shrink-0">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder={localPlayers.length >= 20 ? "Limite de jogadores atingido" : "Nome do participante"}
              disabled={localPlayers.length >= 20}
              maxLength={30}
              className="flex-1 bg-slate-100 border-none rounded-xl px-4 py-3 text-slate-900 focus:ring-2 focus:ring-yellow-400 outline-none transition-all disabled:bg-slate-50 disabled:text-slate-400"
            />
