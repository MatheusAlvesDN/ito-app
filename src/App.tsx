  ...
  useEffect(() => {
    const lockOrientation = async () => {
      try {
        ...
        // @ts-expect-error - window.screen.orientation is experimental
        if (window.screen && window.screen.orientation && typeof window.screen.orientation.lock === 'function') {
          // @ts-expect-error - window.screen.orientation.lock is experimental
...
const RegisterScreen = ({ onBack, onNext }: { onBack: () => void, onNext: (players: string[]) => void }) => {
  ...
  const addPlayer = () => {
    ...
    // Security: Prevent DoS via memory exhaustion/infinite loop
    // Ensure player length limits are enforced
    if (trimmedInput && trimmedInput.length <= 30 && localPlayers.length < 20) {
      ...
      <div className="flex-1 p-6 flex flex-col max-w-full overflow-hidden">
        <div className="bg-white p-4 rounded-3xl shadow-lg mb-6 border border-slate-100 shrink-0">
          <label className="block text-slate-500 font-bold mb-2 ml-1 text-xs uppercase tracking-wider">
            Adicionar Jogador {localPlayers.length >= 20 && <span className="text-red-500 lowercase">(máximo atingido)</span>}
          </label>
