...
const RegisterScreen = ({ onBack, onNext }: { onBack: () => void, onNext: (players: string[]) => void }) => {
  ...
  const addPlayer = () => {
    // 🛡️ SECURITY: Prevent DoS by bounding array size (max 20 players)
      ...
      <div className="flex-1 p-6 flex flex-col max-w-full overflow-hidden">
        <div className="bg-white p-4 rounded-3xl shadow-lg mb-6 border border-slate-100 shrink-0">
          ...
          <div className="flex gap-2">
            <input
              value={inputValue}
              // 🛡️ SECURITY: Disable input when max capacity is reached, enforce maxLength
              disabled={localPlayers.length >= 20}
              maxLength={30}
              onChange={(e) => setInputValue(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && addPlayer()}
              placeholder={localPlayers.length >= 20 ? "Limite máximo alcançado" : "Nome do participante"}
