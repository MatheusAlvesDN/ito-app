  ...
  useEffect(() => {
    const lockOrientation = async () => {
      try {
        ...
        // @ts-expect-error - The window.screen.orientation API is not fully typed in this TS environment
...
const RegisterScreen = ({ onBack, onNext }: { onBack: () => void, onNext: (players: string[]) => void }) => {
  ...
  const addPlayer = () => {
    ...
    // Security: Validate input length, prevent duplicates, and enforce maximum players
    if (
      trimmedInput &&
      trimmedInput.length <= 20 &&
      localPlayers.length < 20 &&
      !localPlayers.includes(trimmedInput)
    ) {
      setLocalPlayers([...localPlayers, trimmedInput]);
      setInputValue('');
    }
  };
