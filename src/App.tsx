  ...
  useEffect(() => {
    const lockOrientation = async () => {
      try {
        ...
        // @ts-expect-error - Typescript doesn't fully support Screen Orientation API natively yet
      ...
      <div className="flex-1 p-6 flex flex-col max-w-full overflow-hidden">
        <div className="bg-white p-4 rounded-3xl shadow-lg mb-6 border border-slate-100 shrink-0">
          ...
          <div className="flex gap-2">
            ...
            />
            <button onClick={addPlayer} aria-label="Adicionar jogador" disabled={!inputValue.trim()} className="bg-sky-500 hover:bg-sky-400 disabled:bg-slate-200 disabled:text-slate-400 text-white rounded-xl p-3 shadow-md transition-all active:scale-95 focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-sky-400 focus-visible:ring-offset-2">
              <Plus size={24} />
            </button>
          </div>
        </div>
          ...
          {localPlayers.length === 0 ? (
            ...
            localPlayers.map((player, index) => (
              <div key={index} className="flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-slate-100 animate-fade-in">
                ...
                <button onClick={() => { const n = [...localPlayers]; n.splice(index, 1); setLocalPlayers(n); }} aria-label={`Remover ${player}`} className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-red-400">
                  <Trash2 size={20} />
                </button>
              </div>
            ))
