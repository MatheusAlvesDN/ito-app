  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 relative h-full">
      <div className="p-4 flex items-center bg-white shadow-sm sticky top-0 z-20 pt-8 md:pt-4 safe-top">
        <button onClick={onBack} aria-label="Voltar" className="p-2 bg-slate-100 rounded-full hover:bg-slate-200 transition-colors focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-sky-500"><ChevronLeft size={24} className="text-slate-700" /></button>
      ...
      <div className="flex-1 p-6 flex flex-col max-w-full overflow-hidden">
        <div className="bg-white p-4 rounded-3xl shadow-lg mb-6 border border-slate-100 shrink-0">
          ...
          <div className="flex gap-2">
            ...
            />
            <button onClick={addPlayer} disabled={!inputValue.trim()} aria-label="Adicionar jogador" className="bg-sky-500 hover:bg-sky-400 disabled:bg-slate-200 disabled:text-slate-400 text-white rounded-xl p-3 shadow-md transition-all active:scale-95 focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-sky-500">
              <Plus size={24} />
            </button>
          </div>
        </div>
          ...
          {localPlayers.length === 0 ? (
            ...
            localPlayers.map((player, index) => (
              <div key={index} className="flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-slate-100 animate-fade-in">
                ...
                <button onClick={() => { const n = [...localPlayers]; n.splice(index, 1); setLocalPlayers(n); }} aria-label={`Remover ${player}`} className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-red-500">
                  <Trash2 size={20} />
                </button>
              </div>
            ))
