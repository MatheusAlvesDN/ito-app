...
const RegisterScreen = ({ onBack, onNext }: { onBack: () => void, onNext: (players: string[]) => void }) => {
  ...
  const addPlayer = () => {
    // 🛡️ SECURITY: Prevent memory exhaustion and infinite loop DoS (in GameScreen) by limiting max players to 20
        ...
        <div className="bg-white p-4 rounded-3xl shadow-lg mb-6 border border-slate-100 shrink-0">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder={localPlayers.length >= 20 ? "Limite atingido (20)" : "Nome do participante"}
              // 🛡️ SECURITY: Limit input length to prevent memory exhaustion
              maxLength={30}
