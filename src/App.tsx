  ...
  useEffect(() => {
    const lockOrientation = async () => {
      try {
        ...
        // @ts-expect-error - The screen orientation API is not fully typed in standard DOM types
        if (window.screen && window.screen.orientation && typeof window.screen.orientation.lock === 'function') {
          // @ts-expect-error - The lock method might not be recognized
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 relative h-full">
      <div className="p-4 flex items-center bg-white shadow-sm sticky top-0 z-20 pt-8 md:pt-4 safe-top">
        <button aria-label="Voltar" onClick={onBack} className="p-2 bg-slate-100 rounded-full hover:bg-slate-200 transition-colors focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:outline-none"><ChevronLeft size={24} className="text-slate-700" /></button>
